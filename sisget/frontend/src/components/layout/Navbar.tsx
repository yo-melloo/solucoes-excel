...
export default function Navbar() {
  ...
  return (
    <header className={`h-20 bg-[var(--navbar-bg)] backdrop-blur-md border-b border-[var(--border)] fixed top-0 ${isSidebarCollapsed ? 'left-20' : 'left-64'} right-0 z-40 px-8 flex items-center justify-between transition-all duration-300`}>
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-2 relative">
          ...
          <button 
            ...
            title={`Alternar tema: ${theme === 'dark' ? 'Escuro' : theme === 'light' ? 'Claro' : 'Automático'}`}
          >
            {getThemeIcon()}
          </button>
        </div>
